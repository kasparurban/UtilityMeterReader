...
namespace MeterReader.Xamarin
{
    public class MeterReader
    {
        ...
        {
            _settings = settings;
            _stringExtractor = new ImageStringExtractor(settings.TesseractApi);
        }

        public async Task Init()
        {
            await _stringExtractor.Init();
        }
        
        public void SetSectorColor(bool darkSectors)
        {
            _settings.DarkSectors = darkSectors;
        }

        public async Task<Reading> Analyze(Mat input)
        {
            ...
            var reading = new Reading(InsertDecimal(await _stringExtractor.GetStringFromImage(numbers)),
